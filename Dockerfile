...
RUN mkdir -p /usr/src/ingc_geonode_theme

WORKDIR /usr/src/ingc_geonode_theme
...
COPY . /usr/src/ingc_geonode_theme

RUN chmod +x /usr/src/ingc_geonode_theme/tasks.py \
    && chmod +x /usr/src/ingc_geonode_theme/entrypoint.sh

# app-specific requirements
RUN pip install --upgrade --no-cache-dir --src /usr/src -r requirements.txt
RUN pip install --upgrade -e .

ENTRYPOINT ["/usr/src/ingc_geonode_theme/entrypoint.sh"]
